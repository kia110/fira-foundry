...
interface IUsd0PP is IERC20Metadata {
    ...
    function setupEarlyUnlockPeriod(uint256 bondEarlyUnlockStart, uint256 bondEarlyUnlockEnd) external;
    ...
    function mintWithPermit(uint256 amountUsd0, uint256 deadline, uint8 v, bytes32 r, bytes32 s) external;
    ...
    /// @dev This function performs a series of actions: it checks for sufficient USD0 balances, exchanges USD0 for
    /// USD0++, and unwraps USD0++ back to USD0, which is then burned and any excess sent to the treasury.
    ///      The function reverts if the PAR mechanism does not result in more USD0++ gained overall than USD0 is
    /// exchanged for. @param parUsd0Amount The amount of USD0 to be exchanged for USD0++.
    /// @param minimumPARMechanismGainedAmount The minimum additional amount of USD0++ expected to gain on top of the
    /// exchanged amount to account for slippage.
    function triggerPARMechanismCurvepool(uint256 parUsd0Amount, uint256 minimumPARMechanismGainedAmount) external;
    ...
    /// @notice Allocates early unlock amounts to specified addresses, allowing them to early exit bonds during the
    /// temporary exit period. @dev Only callable by accounts with the `EARLY_BOND_UNLOCK_ROLE`.
    ...
    /// @notice Set the disabled status of a user's temporary redemptions. Only callable by the airdrop tax collector
    /// contract. @param user The address of the user to disable temporary redemptions for.
    ...
    /// @notice Gets the temporary unlock start time
    /// @return The temporary unlock start time
    function getTemporaryUnlockStartTime() external view returns (uint256);

    /// @notice Gets the temporary unlock end time
    /// @return The temporary unlock end time
    function getTemporaryUnlockEndTime() external view returns (uint256);

    /// @notice Gets the allocation early unlock amount for an address
    /// @param addressToCheck The address to check the allocation for
    /// @return The allocation early unlock amount
